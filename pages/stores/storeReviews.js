...
import { Card, Rating, Container, Header } from "semantic-ui-react";
import web3 from "../../ethereum/web3";

class StoreReviews extends React.Component {
  static async getInitialProps(props) {
    const { address } = props.query;
    const store = Store(address);
    const storeReviews = await store.methods.getStoreReviews().call();
    const storeName = await store.methods.storeName().call();
    /*
    ...
    */
    return {
      address,
      storeName,
      writtenReviews: storeReviews[0],
      reviewersAddress: storeReviews[1],
      ratings: storeReviews[2],
    };
  }
